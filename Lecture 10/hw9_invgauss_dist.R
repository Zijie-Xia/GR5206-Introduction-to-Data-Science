...
obj_invgauss<-function(par,x){
  ...
  differences<-(mom_invgauss(par)-moments)^2
  return(sum(differences))
}

par_invgauss1<-function(x){
  return(optim(c(1,1),function(par) obj_invgauss(par,x))$par)
}
...
g1<-ggplot(data=random_data_tbl,aes(sample=random_data))+
  geom_qq(distribution=qinvgauss,dparams = dparams)+
...
nll_invgauss<-function(par,x){
  -sum(dinvgauss(x,mean=par[1],shape=par[2],log = TRUE))
}
set.seed(100)
seed2<-.Random.seed
fit<-optim(c(1,1),function(par) nll_invgauss(par,random_data),hessian=T,lower=0,method="L-BFGS-B")
...
shape_hat<-mean(1/random_data-1/mu_hat)
...
## Do not modify this line!
ks.test(random_data,pinvgauss,par2)
...
g2<-ggplot(data=random_data_tbl,mapping=aes(x=random_data))+
  geom_histogram(bins=15,aes(y=..density..,))+
  geom_density(aes(color = "Data"))+
  stat_function(fun=function(x) dinvgauss(x,par1[1],par1[2]),aes(color = "MOM"))+
  stat_function(fun=function(x) dinvgauss(x,par2[1],par2[2]),aes(color = "MLE"))+
  stat_function(fun=function(x) dinvgauss(x,2,3),aes(color = "True Value"))+
