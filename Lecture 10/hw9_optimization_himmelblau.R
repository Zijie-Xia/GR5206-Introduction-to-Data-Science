...
himmelblau_plot<-wireframe(fnxy ~ x * y, 
          data=xy, 
          shade = TRUE, 
          scales=list(arrows = FALSE),
          screen=list(z=-300, x=-70, y=0),
          xlab='X',
          ylab='Y',
          zlab='Z')
himmelblau_plot
...
contour<-ggplot(data=xy,aes(x = x, y = y, z = fnxy))+
...
softline_dir<-crossing(x = c(1, -1),y = c(1, -1))%>%
  mutate(d0=map2(x, y, c),
         a=map_dbl(d0,function(d0)softline(x0,d0,himmelblau)),
