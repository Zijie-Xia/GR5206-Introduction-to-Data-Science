...
g1<-ggplot(data=x,aes(x=x))+
  stat_function(fun=function(x){(2400-50*x)/24})+
  stat_function(fun=function(x){(2100-30*x)/33})+
  ...
  geom_point(aes(x=45,y=5),color='red')+
  geom_point(aes(x=45,y=(2400-50*45)/24),color='red')+
  geom_point(aes(x=(2400-5*24)/50,y=5),color='red')+
...
g2<-g1+
  ...
  stat_function(fun=function(x){-x+45+(2400-50*45)/24},color='blue')+
  stat_function(fun=function(x){-x+(2400-5*24)/50+5},color='blue')
...
library(lpSolve)
const.mat=matrix(c(-50,-24,-30,-33,1,0,0,1),ncol=2,byrow=TRUE)
const.mat

s1=lp(direction = "max", objective.in = rep(1, 2),
      const.mat = const.mat,
      const.dir =rep(">=", 4),
      const.rhs = c(-2400,-2100,45,5))
...
s2<-lp(direction='max',objective.in=c(18,25,10,12,15),
       const.mat=const.mat,
       const.dir=rep("<=", 4),
       const.rhs=c(160,200,120,280),all.int = T)