...
library(tidyverse)
value_of_params<-de_ackley$member$bestmemit%>%
  as_tibble()%>%
  rename(x=par1,y=par2)%>%
  mutate(iteration=row_number(),
         objective=f_ackley(x,y))
value_of_params
...
library(ggplot2)
g1<-ggplot(data=value_of_params,mapping=aes(x=x,y=y,color=objective))+
  geom_line()+
...
g2<-ggplot(data=value_of_params_pivot,mapping=aes(x=iteration,y=value,color=name))+
  geom_line()+
  scale_x_log10()+
  geom_hline(yintercept = max(value_of_params$objective),linetype=2)+
