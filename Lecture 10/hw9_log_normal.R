...
set.seed(0)
seed <- .Random.seed
x<-rlnorm(1000,meanlog = 2,sdlog = 1)+rnorm(1000,0,0.05)
...
mom_lnorm<-function(par){
  meanlog<-par[1]
  sdlog<-par[2]
  c(expectation = exp(meanlog + 0.5 * sdlog^2),
    variance = exp(2 * meanlog + sdlog^2) * (exp(sdlog^2) - 1))
}
mom_lnorm(c(1, 1))
...
par_lnorm<-function(x){
  est<-optim(c(1, 1),obj_lnorm_factory(x),hessian=TRUE)
  par<-est$par
  h<-est$hessian
  ...
  print(par)
  print(sd)
  t<-tibble(parameter=c("mu_mom","sd_mom"),
         value=c(par[1],par[2]),
         se=c(sd[1],sd[2]),
         lower=c(value-1.96*se),
         upper=c(value+1.96*se))
  return(t)
}
result_mom<-par_lnorm(x)
result_mom
mu_mom<-result_mom[[1,2]]
sd_mom<-result_mom[[2,2]]
...
nll_lnorm<-function(par,x){
  distribution<-dlnorm(x,par[1],par[2],log=T)
  log_like<--sum(distribution)
  return(log_like)
}
optimization<-optim(c(1,1),nll_lnorm,x=x)$par
mu_mle<-optimization[1]
sd_mle<-optimization[2]
...
library(ggplot2)
x<-as.data.frame(x)
compare_plot<-ggplot(data=x,aes(x=x))+
  geom_histogram(aes(y=..density..),bins=50)+
  geom_density(aes(color="Data"))+
  stat_function(fun=function(x) dlnorm(x,mu_mom,sd_mom),aes(color="MOM"))+
  stat_function(fun=function(x) dlnorm(x,mu_mle,sd_mle),aes(color="MLE"))+
  labs(title="Both methods seem to do a good job",
       x = "Simulated x",
       y = "Density")+theme_light()+theme(plot.title=element_text(hjust=0.5))
compare_plot
