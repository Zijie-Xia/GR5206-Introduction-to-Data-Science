...
data<-read_csv('/home/data/kc_house_data.csv')
data
...
data_clean<-data%>%
  mutate(waterfront=factor(waterfront,labels=c('No','Yes')),
         ...
         age=2019-yr_built,
         renovated=factor((yr_renovated!=0),labels=c('No','Yes')))%>%
  dplyr::select(-c(yr_built, yr_renovated, id,date))
data_clean
...
#    side-by-side.
## Do not modify this line!
g1<-ggplot(data=data_clean,aes(x=waterfront,y=price))+
  geom_boxplot()+
  coord_flip()+
...
g2<-ggplot(data=data_clean,aes(x=renovated,y=price))+
  geom_boxplot()+
  coord_flip()+
...
g3<-ggpairs(data=select(data_clean,c(price,sqft_lot,sqft_above,sqft_living,sqft_basement)))+
...
data_model<-data_clean%>%
  dplyr::select(-c(sqft_lot,quarter,zipcode,sqft_above,sqft_basement,sqft_living15,sqft_lot15))
...
fit_m1<-data_model%>%
  add_residuals(m1)%>%
  add_predictions(m1)%>%
  rename(resids=resid,preds=pred)%>%
...
## Do not modify this line!
g4<-ggplot(data=fit_m1,aes(x=preds,y=resids))+
...
g5<-ggplot(data=fit_m1,mapping=aes(sample=resids))+
  stat_qq()+
  stat_qq_line()+
  labs(title = "Residuals Are Almost Normal",
       subtitle = "Potential outliers are seen at the heavy tail.",
       x = "Theoretical Quantiles",
       y = "Sample Quantiles")+
  theme_light()
...
g6<-ggplot(data=fit_m2,aes(x=preds,y=resids))+
  geom_point(alpha = 0.3)+
  geom_hline(yintercept = 0,color = "red")+
  labs(title = "Residuals Have No Pattern",
       ...
       y = "Residuals ($)")+  
  theme_light()
g7<-ggplot(data=fit_m2,mapping=aes(sample=resids))+
  stat_qq()+
  stat_qq_line()+
...
by_quarter<-data_clean%>%
  dplyr::select(-c(zipcode,sqft_above,sqft_lot,sqft_basement,sqft_living15,sqft_lot15))%>%
...
#           # … with 2.036e+04 more rows
## Do not modify this line!
by_quarter_resid<-by_quarter%>%
  unnest(resids)%>%
  mutate(key=row_number())%>%
  dplyr::select(quarter,key,resid)

by_quarter_pred<-by_quarter%>%
  unnest(preds)%>%
  mutate(key=row_number())%>%
  dplyr::select(quarter,key,pred)

by_quarter_fit<-inner_join(by_quarter_resid,by_quarter_pred,by=c("quarter", "key"))
by_quarter_fit
...
g8<-ggplot(data=by_quarter_fit,aes(x=pred,y=resid))+
  geom_point(alpha = 0.3)+
  geom_hline(yintercept = 0,color = "red")+
  facet_wrap(~quarter)+
  labs(title = "Residuals Have No Pattern",
