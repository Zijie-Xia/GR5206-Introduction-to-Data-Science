...
temperature<-temperature_original%>%
  mutate(year=lubridate::year(date),month=factor(lubridate::month(date,label=T),levels = month.abb[1:12],ordered = F))%>%
  filter(year>=1900)%>%
  drop_na()
...
temperature_change<-temperature%>%
  filter((year>1910 & year <2010) | (year==1910 & month %in% month.abb[3:12]) | (year==2010 & month %in% month.abb[1:3]))%>%
  group_by(country)%>%
  arrange(year)%>%
  summarize(temperature=rev(temperature)[1]-temperature[1])
temperature_change
...
temperature_map<-ggplot(temperature_change,aes(map_id=country))+
...
switzerland_temperature_plot<-ggplot(data=sw,aes(x=month,y=temperature))+
  geom_boxplot()+
...
sw_seasonal_plot<-switzerland_temperature_plot+
  geom_point(data=sw_seasonal,color="green",size=2,pch=3)+
...
## Do not modify this line!
residuals_plot<-ggplot(data=sw_residuals,aes(x=year,y=resid))+
  geom_point()+
  geom_smooth(method='lm',col='red',size=2,se=F)+
...
## Do not modify this line!
sw_improved_mod<-lm(temperature~year+month,data=sw)
...
monthly_pattern_plot<-ggplot(data=monthly_pattern,aes(x=month,y=pred,color=country,group=country))+
  geom_point()+
  geom_line()+
