...
temperature_full<-temperature%>%
  pivot_longer(2:5, names_to = "city",values_to = "temperature")%>%
...
temperature_ny<-temperature_full%>%
...
ny_temp<-ggplot(data=temperature_ny,aes(x=datetime,y=temperature))+
  geom_line()+
...
ny_temp_fitted<-ggplot(data=temperature_ny_model0,aes(x=datetime,y=temperature))+
  geom_point(size=0.2,alpha=0.2)+
  geom_point(aes(y=pred),size=0.1,col='red')+
...
ny_temp_fitted2<-ggplot(data=temperature_ny_model0,aes(x=datetime,y=temperature))+
  geom_point(size=0.2,alpha=0.2)+
  geom_point(aes(y=pred),size=1,col='red')+
  geom_line(data=temperature_ny_model1,aes(y = pred),color="green",size=1)+
  labs(title="The seasonality is accounted for",
       y="Temperature (C)",
       x="Date",
       subtitle="Polynomials in red miss the cyclicality, but cyclic splines in
...
temperature_ny_model2<-
  left_join(gather_predictions(temperature_ny,model1,model2),gather_residuals(temperature_ny,model1,model2))%>%
...
by_city_pred<-by_city%>%
  mutate(pred=map(model,~predict(.x,newdata=test_df)))%>%
  mutate(pred=map(pred,enframe))%>%
  mutate(pred=map(pred,~bind_cols(.x,test_df)))%>%
  unnest(pred)%>%
  mutate(pred=value)%>%
  dplyr::select(c(city     ,  pred, yearfrac,  hour))%>%
  mutate(yearfrac=fct_recode(factor(yearfrac),Winter='0',Spring='0.25',Summer='0.5',Autumn='0.75'),hour=as.numeric(hour)-1)
...
temperature_per_hour_season_plot<-ggplot(data=filter(by_city_pred,city %in% c('Miami','Phoenix','Vancouver')),
       aes(x=hour,y=pred))+
  geom_point()+
  facet_grid(city~yearfrac)+
  labs(title="Our model can predict the temperature for different seasons!",
       x="Hour",
       y="Prediction (C)")

temperature_per_hour_season_plot
