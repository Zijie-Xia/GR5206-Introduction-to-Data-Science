...
quad_resid_plot<-ggplot(data=data_with_quad_resid,aes(x = x, y= resid))+
  geom_ref_line(h=0)+
  geom_point()+
  labs(y = "Residuals",
       title = "The residuals show a secondary pattern that is not captured")
...
model_pred_resid<-1:10%>%
  as_tibble()%>%
  rename(k=value)%>%
  mutate(model=map(k,my_model))%>%
  mutate(data=map(model,function(x) add_residuals(add_predictions(all_data,x),x)))%>%
  unnest(data)%>%
  select(-model)
...
plot_mse<-ggplot(data=model_mse,aes(x=k,y=mse,color=split))+
  geom_line()+
...
residual_plot<-ggplot(filter(model_pred_resid,k == best_deg),
                      aes(x=x,y=resid))+
  geom_point()+
...
## Do not modify this line!
m<-model_pred_resid%>%
  filter(k %in% best_deg | k==7)%>%
  mutate(k=factor(k))
prediction_plot<-ggplot(data=m,aes(x=x,y=y))+
  geom_point()+
...
model_mse_penalized<-model_pred_resid%>%
  group_by(k,split)%>%
  nest()%>%
  mutate(errors=map(data, function(x) my_mse_penalized(x,k,lambda=c(0, 0.1, 0.4))))%>%
  unnest(errors)%>%
  select(-data)
model_mse_penalized
...
## Do not modify this line!
model_mse_penalized2<-model_mse_penalized%>%
  mutate(lambda=factor(lambda))
mse_penalized_plot<-ggplot(data=model_mse_penalized2,
                           aes(x=k,y=loss,color=split,linetype=lambda))+
  geom_line()+
  labs(color = "Split",
       x = "Polynomial Degree",
       y = "Loss",
...
best_deg_penalized<-model_mse_penalized%>%
  filter(split=='test')%>%
  group_by(lambda)%>%
  filter(loss==min(loss))

best_deg_penalized
model_pred_resid_best<-left_join(model_pred_resid,best_deg_penalized,by='k')%>%
  drop_na()
...
residual_plot_best<-ggplot(model_pred_resid_best,aes(x=x,y=resid))+
  geom_point()+
  geom_hline(yintercept=0,size=1)+
  facet_wrap(~k)+
  labs(y = "Residuals",
