...
## Do not modify this line!
library(stats)
quantile1<-function(x) stats::quantile(x,0.25)
quantile3<-function(x) stats::quantile(x,0.75)
summary_stats<-iris%>%
  select(-Species)%>%
  summarize_if(is.numeric,funs(Min.=min,'1st Qu.'=quantile1,Median=median,Mean=mean,'3rd Qu.'=quantile3,Max.=max))
summary_stats
...
summary_long<-summary_stats%>%
  pivot_longer(Sepal.Length_Min.:Petal.Width_Max.,names_to='combi',values_to='value')%>%
  separate(combi,into=c('variable','stat'),sep='_')
summary_long 
