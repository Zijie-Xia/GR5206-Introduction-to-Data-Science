...
stats_by_state<-mobility%>%
  group_by(State)%>%
  summarize_at(vars(Commute,Mobility),c(mean=mean,sd=sd))
stats_by_state
...
stats_by_state_longer<-stats_by_state%>%
  pivot_longer(Commute_mean:Mobility_sd,names_to = 'Stat',values_to = 'Value')%>%
...
coordinates<-mobility%>%
  group_by(State)%>%
  summarize(Latitude_mean=round(mean(Latitude,na.rm=T),digits=3),Longitude_mean=round(mean(Longitude,na.rm=T),digits=2))%>%
  unite(Coordinates,Latitude_mean,Longitude_mean,sep='_')
coordinates
