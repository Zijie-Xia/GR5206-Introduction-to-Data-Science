...
t2<-billboard%>%
  as_tibble()%>%
  pivot_longer(starts_with('wk'),names_to = 'week',names_prefix='wk',values_to = 'rank',values_drop_na=T)
t2
...
## Do not modify this line!
t2_rank<-t2%>%
  mutate(diff=lag(rank)-rank)%>%
  filter(week!='1')%>%
  group_by(artist,track,date.entered)%>%
  summarize(highest_rank_increase=max(diff))
t2_rank
...
t5<-billboard%>%
  ...
  arrange(wk1)%>%
  rename(wk1_rank=wk1)%>%
  select(artist,track,date.entered,wk1_rank)
t5
...
## Do not modify this line!
t6<-t1%>%
  filter(startsWith(as.character(date.entered),'2000'))%>%
  group_by(artist)%>%
