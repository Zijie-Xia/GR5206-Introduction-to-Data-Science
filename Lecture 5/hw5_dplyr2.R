...
ea_na_75_85<-longevity%>%
  filter(((region=='East Asia & Pacific')|(region=='North America'))&((life_expect>=75)&(life_expect<=85)))%>%
...
top_10_perc_us<-longevity%>%
  ...
  filter((perc_rank>=quantile(perc_rank,0.9))|(country=="United States"))
top_10_perc_us
...
gdp_by_region<-nations%>%
  ...
  summarise(total_gdp=sum(gdp,na.rm = T)/1000000000000)
gdp_by_region
...
p_countries<-nations%>%
  filter(year==2016)%>%
  group_by(income)%>%
  summarise(p=sum(life_expect>70,na.rm=T)/n())
p_countries
