...
library(dplyr)
years<-trips%>%
  summarize(min_birth_year=min(birth_year,na.rm = T),max_birth_year=max(birth_year,na.rm = T))
years
...
## Do not modify this line!
broadway<-trips%>%
  filter(str_detect(trips$start_station_name,'Broadway')|str_detect(trips$end_station_name,'Broadway'))
broadway
