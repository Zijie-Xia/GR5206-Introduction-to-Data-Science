...
new_distance<-function(x, units){
	stopifnot(is.numeric(x))
	stopifnot(is.character(units))
	dis<-structure(x,class="distance",units=units)
	return(dis)
}
distance<-function(x, units="m"){
	stopifnot(units %in% c("mm", "cm", "m", "km"))
	new_distance(x,match.arg(units,c("mm", "cm", "m", "km")))
}
print.distance<-function(x,...){`
	uni<-as.character(attr(x,"units"))
	if (length(x)==1) print(paste0("the distance is ",x," ",uni))
	else print(paste0("the distances are ",paste0(x,collapse=" ")," ",uni))
	invisible(x)
}
dist1<-distance(10)
print(dist1)
...
## Do not modify this line!
to_mm<-function(x){ 
	n<-switch(attr(x,"units"),"mm"=0,"cm"=1,'m'=3,'km'=6)
	if(is.null(n)) stop("Unknown unit.")
	y<-x*10^n
	distance(y,'mm')
}
...
mean.distance<-function(x,...){
	m<-mean(as.numeric(to_mm(x)))
	distance(m,"mm")
}
avg<-distance(mean(dist_both),"mm")
avg
