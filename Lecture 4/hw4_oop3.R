...
convert.weight<-function(x){
	if(attr(x,"units")=="kg") return(weight(x*2.2,"lbs"))
	else return(weight(x/2.2,"kg"))
}
d2<-convert(d1)
d2
...
## Do not modify this line!
`+.weight`<-function(x,y){
	if(attr(x,"units")!=attr(y,"units")) y<-convert(y)
	weight(NextMethod("+.double"),units=attr(x,"units"))
}
