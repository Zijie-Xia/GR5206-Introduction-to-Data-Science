...
safe_max<-safely(max)
y<-map(iris,safe_max)
not_ok<-map_lgl(transpose(y)$result,is_null)
is_ok<-!not_ok
...
#    vector `m2`.
## Do not modify this line!
invalid_column<-colnames(iris)[not_ok]
r<-(transpose(y[is_ok]))$result
m2<-flatten_dbl(r)
...
## Do not modify this line!
poss_max<-possibly(max,NA)
m3<-map_dbl(iris,poss_max)
...
## Do not modify this line!
s<-map_dbl(iris,possibly(mean,otherwise=NA))
...
warning_log
is_ok2<-map_lgl(warning_log,~length(.x)==0)
