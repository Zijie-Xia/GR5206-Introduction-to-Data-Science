...
spotify<-spotify%>%
  mutate(popularity_label=fct_recode(cut(spotify$popularity,breaks=c(0, 30, 70, 100)),
                                     unpopular = "(0,30]",fair = "(30,70]",popular = "(70,100]"))%>%
  drop_na()
...
## Do not modify this line!
dparams_tempo<-list(mean=tempo_bts[1],
                    sd=sqrt(tempo_bts[2]))
...
liveness_pop_density1<-partial(dgamma, 
                               shape =liveness_pop1[1], 
                               scale=liveness_pop1[2])

liveness_pop_density2<-partial(dgamma, 
                               shape =liveness_pop2[1], 
...
liveness_gamma_plot<-ggplot(data=spotify_pop)+
  geom_histogram(aes(x = liveness, y = ..density..),
                 binwidth=0.02,
                 color="black",
                 fill="orange")+
  geom_density(aes(x = liveness),
               kernel="gaussian",
               bw=0.06,
               color="red",
               fill="red",
               alpha=0.5)+
  stat_function(fun = liveness_pop_density1,
                color="yellow")+
  stat_function(fun = liveness_pop_density2,
                color="green")+
  labs(x="Liveness",
       y="Density",
