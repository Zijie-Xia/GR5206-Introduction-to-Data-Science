...
multi_step<-function(f,x,h,n_iterations){
  t<-accumulate(rep(h,n_iterations),function(x,h)one_step(f,x,h),.init = x)
  enframe(t,name = 'step',value = 'x')
}
multi_step(f, 1, h, 2)
...
newton_steps_tibble<-tibble(start = seq(-7, 7, 2))%>%
  mutate(steps = map(start,~multi_step(f,.x,h,n_iterations = 2)))%>%
  unnest(steps)%>%
  mutate(f=f(x),
         step=as.factor(step),
...
newton_steps_plot<-ggplot(data=newton_steps_tibble,aes(x=x,y=f))+
  stat_function(fun = f)+
...
plot_with_root<-f_plot+
  geom_point(aes(x=-3,y=f(root)),colour ="red")+
