...
## Do not modify this line!
stock_return_plot<-ggplot(data=stocks_data_longer,
                          aes(x=date,y=return,group=stock))+
...
dvec<-rep(0,10)
A<-matrix(rep(1,10),ncol=1)
mu<-matrix(colMeans(stocks_data[2:11]),10,1)
Amat=t(cbind(A,mu,diag(10),-diag(10)))
...
portfolio<-tibble(stock=names(stocks_data)[-1],
       proportion=round(sol[[1]],3))%>%
...
portfolio_plot<-ggplot(data=portfolio,aes(x=stock,y=proportion))+
