...
g1<-ggplot(data=data,aes(x=count))+
  geom_histogram()+
...
par_pois2<-function(x){
  ...
  fit<-optim(par0,nll_pois_x,method = "L-BFGS-B",hessian = TRUE,lower = c(0.001, 0),upper = c(Inf, 0.999))
  fisher_info<-solve(fit$hessian)
  se<-sqrt(diag(fisher_info))
  error<-1.96*se
  lower<-fit$par-error
  upper<-fit$par+error
  t<-tibble(parameter=c('lambda','pi'),value=fit$par,se=se,lower=lower,upper=upper)
  return(t)
}
...
g3<-ggplot(data=data,aes(sample =count))+
