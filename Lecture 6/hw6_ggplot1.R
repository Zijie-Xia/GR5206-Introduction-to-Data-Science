...
library(readr)
library(tidyr)
abalone<-drop_na(read_csv("data/abalone.csv"))
abalone
...
g4<-ggplot(data = abalone,mapping = aes(x = rings,y=diameter)) +
  geom_point()+
...
g5<-ggplot(data = abalone) +
  geom_violin(mapping = aes(x = length,y=whole_weight))+
  facet_wrap(~sex)+
