...
library(ggplot2)
g1<-ggplot(data = iris)+
  geom_point(mapping = aes(x=Sepal.Length,y=Petal.Length,color=iris$Species))+
...
## Do not modify this line!
g3<-g2+
  geom_smooth(mapping=aes(x=Sepal.Length,y=Petal.Length,color=iris$Species),method = "lm",se = FALSE,fullrange = TRUE)+
...
g4<-g3+
  geom_point(mapping=aes(x=iris$Sepal.Length[15],y=iris$Petal.Length[15]),pch = 17,col = "black",cex = 3)+
  theme_light()+
  theme(plot.title=element_text(hjust=0.5),plot.subtitle=element_text(hjust=0.5))
...
g5 <- g4 +
  geom_text(mapping = aes(x=iris$Sepal.Length[15],y=iris$Petal.Length[15],label = "This is the iris with the longest sepal\namong the setosa species."),col = "black", nudge_x = 1)+
  theme_light()+
  theme(plot.title=element_text(hjust=0.5),plot.subtitle=element_text(hjust=0.5))
g5
