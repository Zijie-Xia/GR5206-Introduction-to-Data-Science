...
g1<-ggplot(data=taxi,mapping=aes(x=fare_amount,y=tip_amount))+
  geom_point(mapping=aes(stroke = 0),alpha = 0.5)+
...
g3<-ggplot(data=taxi,mapping=aes(x=trip_distance,y=tip_amount,color=payment_type))+
  geom_point(alpha = 0.5,stroke = 0)+
  coord_cartesian(xlim = c(0, 25),ylim = c(0, 15))+
  labs(title =  "Tip Amount Increases with Trip Distance Generally",
       subtitle = paste("Trips with tips were almost always paid in card.","Trips paid in cash were rarely tipped.", sep="\n"),
       x = "Trip distance (mile)",
       y = "Tip amount ($)")+
  theme_light()+
...
taxi_new<-taxi%>%
  mutate(pickup_time=format(tpep_pickup_datetime,"%H"))%>%
  dplyr::select(tip_amount,pickup_time)
taxi_new 
