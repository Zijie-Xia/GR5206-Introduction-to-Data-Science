...
wine<-as_tibble(read_csv('data/winequalityN.csv'))
wine<-drop_na(wine)
wine
...
red<-wine%>%
  filter(type=="red")%>%
  mutate(quality_label=cut(quality,c(2,5,7,10)))%>%
  mutate(quality_label=plyr::revalue(quality_label,c("(2,5]"="bad", "(5,7]"="average" ,"(7,10]"="good")))
...
plot4<-ggplot(data=red,aes(x =`citric acid`,y=density))+
