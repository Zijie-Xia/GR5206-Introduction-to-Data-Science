...
taxi<-read_csv("data/taxi.csv")%>%
  filter(tip_amount < 15 & fare_amount >=0 & fare_amount <= 50 )
...
tip_histogram<-ggplot(data=taxi)+
  geom_histogram(mapping = aes(x=tip_amount),binwidth = 1)+
...
taxi_binned=taxi%>%
  mutate(fare_bins=cut(fare_amount,seq(0,50,10),labels=c("0-10","11-20","21-30","31-40","41-50"),include.lowest = T))
...
tip_facet_hist<-ggplot(data=taxi_binned)+
  geom_histogram(mapping=aes(x=tip_amount),binwidth=1)+
  scale_y_continuous(label = comma)+
  facet_grid(~fare_bins)+
  labs(title = "The tip skew becomes less severe as fares increase",
       x = "Tip Amount (USD)",
       y = "")+
  theme_light()
