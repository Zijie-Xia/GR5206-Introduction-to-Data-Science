...
plot1<-ggplot(data=fu1,mapping=aes(x=home_team,y=total,fill=home_team))+
...
## Do not modify this line!
fu2<-fu%>%
  dplyr::select(home_team,home_score,date)%>%
  mutate(year=year(date))%>%
  filter(year>2013 & year<=2019 &home_team=="England")
...
library(ggridges)
plot2<-ggplot(data=fu2,mapping = aes(x=year,y=home_score,group= home_score,fill=home_score))+
...
fu3<-fu%>%
  select(home_team, tournament, date)%>%
  mutate(tournament=factor(tournament,levels=c('FIFA World Cup', 'UEFA Euro', 'UEFA Nations League', 'Nations Cup','Kirin Cup')))%>%
  drop_na()%>%
  filter(home_team%in%c('England', 'Germany', 'France', 'Belgium', 'Italy' ,'Spain')&year(date)>2000)
...
fu4<-fu%>%
  ...
  group_by(team)%>%
  distinct(year(date))%>%
  select(year=`year(date)`,team)%>%
  mutate(attendance=length(unique(year)))%>%
  filter(attendance>8)%>%
