...
library(ggrepel)
library(readr)
autompg<-as_tibble(read_csv("data/autompg.csv"))
...
## Do not modify this line!
g1 <- ggplot(efficiency_by_manufacturer)+
  geom_boxplot(aes(x=reorder(car_made,efficiency,median, .desc = TRUE),y=efficiency))+
  labs(title='Cars made by European and Japanese manufacturers are generally more efficient',
       ...
       x='Car manufacturer', y='Efficiency (mpg / kg)')+
  coord_flip()+
  theme_light()+
...
g2<-ggplot(efficiency_by_manufacturer)+
  geom_point(aes(x=weight,y=mpg,color=model_year),size=2,shape=1,stroke=1.25)+
  labs(x="Weight of a car (kg)",
       y="Miles per gallon")+
  scale_color_discrete(name="Model year of a car
                       (1970 - 1982)")+
  theme_light()
...
g3<-g2+
  geom_smooth(aes(x=weight,y=mpg,linetype="r2"),method="loess",se=T,color="red")+
  guides(linetype = F)+
  theme_light()
...
g4<-g3+
  geom_text_repel(aes(x=weight,y=mpg,label=car_made),data=best_efficiency_by_manufacturer)+
