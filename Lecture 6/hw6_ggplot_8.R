...
age_plot<-ggplot(data=fifa,mapping = aes(x=Age))+
  geom_histogram(mapping=aes(y=..density..),binwidth=2,color="white")+
  geom_density(color="blue")+
  labs(title = "The majority of players are 20 to 30 years old",
       y = "Density")+
  theme_light()
...
club_wage<-fifa%>%
  filter(Club %in% c("FC Barcelona", "Juventus", "Manchester City", "Real Madrid","Manchester United"))%>%
...
wage_plot<-ggplot(data=club_wage,mapping = aes(x=Club,y=Wage))+
  ...
  labs(title = "Top 5 highest median wage clubs",
       subtitle = "The medians are remarkably similar, but the higher quantiles show more variation.",
       y = "Wage",
       x = "")+
  theme_light()+
  coord_flip()
...
## Do not modify this line!
height_weight_plot<-ggplot(data=fifa_height_weight)+
  geom_point(mapping = aes(x=Height_m,y=Weight_kg,color=Position),alpha=0.8)+
  labs(title = "GK tends to be heavier and taller",
       y = "Weight (kg)",
       x = "Height (m)")+
  theme_light()
...
agility<-fifa%>%
  filter(`Preferred Foot` %in% c("Left","Right"))
...
agility_plot<-ggplot(data=agility)+
  geom_histogram(mapping=aes(x=Agility),bins=20,color="#FFFFFF")+
  facet_wrap(~`Preferred Foot`, scales = "free")+
  labs(title = "Distribution of agility is similar regardless of preferred foot",
       y = "Count (n)")+
  theme_light()
