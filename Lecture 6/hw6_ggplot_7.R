...
mean_value<-wine%>%
  ...
  dplyr::rename("Property"=name,"Value"=value)%>%
...
mean_value_plot<-ggplot(data=mean_value)+
  geom_point(mapping = aes(x=Value,y=Property))+
  scale_x_log10()+
  ggtitle("Most properties have value below 100")+
  theme_light()
...
mean_value_by_type<-mean_value_by_type%>%
  mutate(Property=fct_relevel(as.factor(Property),v))
...
mean_value_plot2<-ggplot(data=mean_value_by_type)+
  geom_point(mapping = aes(x=Value,y=Property,color=factor(Type)))+
  scale_x_log10()+
  labs(color="Type")+
  ggtitle("Mean Proline values vary significantly among three types")+
  theme_light()
