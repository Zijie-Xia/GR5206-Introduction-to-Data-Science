...
library(ggplot2)
library(scales)
salary_boxplots<-ggplot(data=employee)+
  geom_boxplot(mapping=aes(x=Year,y=Salaries))+
  scale_y_continuous(label = comma)+
  labs(title = "Salaries are getting a longer tail with time",
       y = "Yearly salaries (USD)")+
  theme_light()
...
salary_histograms<-ggplot(data=employee_trunc)+
  geom_histogram(mapping=aes(x=Salaries),binwidth = 5000)+
  facet_wrap(~Year)+
...
salary_densities<-ggplot(data=employee_trunc)+
  geom_density(mapping=aes(x=Salaries,color=Year))+
  scale_x_continuous(label = comma)+
...
log_salaries_qq<-ggplot(data=employee_culture,aes(sample=log10(Salaries)))+
  geom_qq()+
  geom_qq_line()+
  labs(title = "The log salaries severely diverge from a normal distribution",
       x = "Theoretical quantiles of log(salaries)",
       y = "Sample quantiles of log(salaries)")+
  theme_light()
