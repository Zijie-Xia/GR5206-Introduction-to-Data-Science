...
#    the resulting tibble to `term_mentions`.
...
#    `example_mentions_counts` should print to:
#    # A tibble: 825 x 5
#       term   year n_mentions total_mentions frac_total
#      <fct> <int>      <int>          <dbl>      <dbl>
...
#    - create a plot from `example_mention_counts` using `ggplot()` with the
...
library(ggplot2)
library(scales)
paper_figure<-ggplot(data=examples_mention_counts,mapping = aes(x=year,y=frac_total,color=term))+
  geom_line()+
