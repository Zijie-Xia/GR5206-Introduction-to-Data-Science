...
g1<-ggplot(data=average_rating_in_different_years,
           aes(x=year_of_rating,y=average_rating_of_year))+
  geom_point()+
  labs(title="2004 and 2005 have lower average ratings",
       subtitle="While 2014 has highest average ratings",
       x="Year",
       y="Average rating")+
  theme_light()+
...
movies<-movies%>%
  mutate(year=map_chr(title,function(x) substr(x,nchar(x)-4,nchar(x)-1)))%>%
...
## Do not modify this line!
ratings_of_movies<-
  inner_join(movies,ratings)%>%
  select(title,genres,year,rating,date)
...
## Do not modify this line!
g2<-ggplot(data=ratings_of_movies,aes(x=genres,y=rating))+
  geom_boxplot()+
...
average_rating_of_genres<-ratings_of_movies%>%
  group_by(genres)%>%
...
g3<-ggplot(data=average_rating_of_genres,aes(x=number_of_ratings,y=average_rating))+
  ...
  labs(title="When number of ratings is over 5000, the average ratings start to be constant around 3.5",
...
ratings_over_time<-ratings_of_movies%>%
  ...
  mutate(years_passed=interval/dyears())%>%
...
g4<-ggplot(data=ratings_over_time,aes(x=years_passed,y=rating,color=genres))+
