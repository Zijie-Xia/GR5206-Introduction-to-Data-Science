...
s_admission_new<-s_admission%>%
  mutate(age=round((DOB%--%today())/dyears(1)),
         school_year=fct_recode(fct_collapse(factor(DOA),"graduate"=c("2013-07-01","2014-07-01")),"senior"="2015-07-01","junior"="2016-07-01","sophomore"="2017-07-01","freshman"="2018-07-01"))%>%
...
department_engie<-department%>%
  filter(str_detect(Department_Name,pattern=fixed("Engineering",ignore_case = T)))%>%
...
## Do not modify this line!
names(employee_new)
names(department_engie)
employee_joined<-employee_new%>%
  inner_join(department_engie,by='Department_ID')%>%
  select(Employee_ID,Department_ID,age,seniority,seniority_level,Department_Name)
...
#    # … with 10 more rows
## Do not modify this line!
department_size<-employee_joined%>%
  dplyr::count(Department_Name)%>%
  arrange(n)
...
employee_joined<-employee_joined%>%
  mutate(Department_Name=factor(Department_Name,levels=pull(department_size,Department_Name)))
employee_joined
...
g1<-ggplot(data=employee_joined)+
  geom_bar(mapping = aes(x=Department_Name,fill=seniority_level))+
  coord_flip()+
  labs(title = "Number of Senior and Junior Professors by Department",
       subtitle = paste("Most departments have more senior professors.","Exceptions are CASDE and Chemical Engineering.",sep = "\n"),
       x = "Department's Name",
       fill = "Seniority Level",
       y = "Count (n)")+
  theme_light()
...
student_joined<-s_admission_new%>%
...
## Do not modify this line!
g2<-ggplot(data=student_joined)+
  geom_boxplot(mapping = aes(x=fct_reorder(school_year,mean_score),y=mean_score))+
  coord_flip()+
...
employee_senior<-employee_joined%>%
...
engie<-student_joined%>%
  inner_join(employee_senior,by=c("Department_Admission"="Department_ID"))%>%
...
## Do not modify this line!
names(engie)
g3<-ggpairs(data=select(engie,seniority,mean_score,min_score,max_score))+
  labs(title = "Correlation between Professor Seniority and Student Academic Performance",
       subtitle = "No significant correlation is found.")+
  theme_light()
