...
room_earning<-booking%>%
  ...
  arrange(booking_date,room_no,guest_id,nights,earning)%>%
...
room_earning_filtered<-room_earning%>%
  filter(room_no<200)%>%
  filter(booking_date>= ymd('2016-11-15') & booking_date<=ymd('2016-12-16'))%>%
...
room_earning_plot<-ggplot(data=room_earning_filtered,mapping=aes(x=booking_date,y=earning))+
  geom_line()+
  scale_x_date(breaks = as.Date(c("2016-11-15", "2016-12-01", "2016-12-15")),date_labels = '%m/%d')+
...
guest_spending_plot<-ggplot(data=guest_spending,mapping = aes(x=name,y=spending))+
  geom_col()+
  coord_flip()+
