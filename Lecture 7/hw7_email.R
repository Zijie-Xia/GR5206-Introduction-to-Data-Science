...
library(lubridate)
email_w_time<-email%>%
  mutate(date=onset-883612800)%>%
  mutate(date=as.POSIXct(date,origin = "1998-01-01"))%>%
  mutate(date=with_tz(date,tz="UTC"))
email_w_time
...
email_id_to_name<-function(email_id){
  name_sp<-str_split(email_id,"\\.")
  name<-map_chr(name_sp,~ paste0(.x,collapse = " "))
  name<-str_trim(name)
  name<-str_to_title(name)
  return(name)
}
email_id_to_name("albert.meyers")
...
t1<-email_w_time%>%
  ...
  rename('receiver_email'='email_id',
           'receiver'='person_name',
           'receiver_role'='role',
           'receiver_dept'='dept')%>%
  left_join(people_new2,by = c("head"="vertex.names"))%>%
  rename('sender_email'='email_id',
           'sender'='person_name',
           'sender_role'='role',
           'sender_dept'='dept')%>%
...
library(ggplot2)
g1<-ggplot(data=t2)+
  geom_histogram(mapping=aes(x=hour),bins = 24)+
...
g2<-ggplot(data=p2,mapping=aes(x=date,y=count,color = sender_role))+
  geom_point()+
