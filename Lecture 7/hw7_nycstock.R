...
raw_time<-raw%>%
  mutate(date=force_tz(date,tz="America/New_York"))
raw_time

# 3. Load the package `forcats`.
#    In `securities`, keep the top 6 sectors that most companies in stock
#    market falls into and whose `GICS Sub Industry` falls into `"Gold"` or
#    `"Real Estate`" (`GICS Sub Industry` contains either `"Gold"` or `"REITs"`).
#    You can do it in two steps:
#      - First, convert the column `GICS Sector` in `securities` from character
#        to factor. Save the clustered factor into new column `GICS Sector truncated`.
#        Create a tibble named `securities_sectored` that deselecting the above
#        conditions on column `GICS Sector` and `GICS Sub Industry`.
#      To do that, you can use:
#        - `unique()` to get the sectors of securities as levels.
#        - `mutate()` and `factor()` to convert the column to factor,
#           `fct_lump()` with `fct_infreq()` to create new column
#           to hold clustered sectors according to their frequencies.
#           Set argument `n` to 6 in `fct_lump()`.
#        - `filter()` to select the sectors that are not top 6 sectors
#           (namely `GICS Sector` == "Other")
#        - `filter()` to select the sub industry in the "Other" sectors that are
#           neither "Gold" nor matching "REITs". You can use `str_detect()` with
#           regular expression (with alternation symbol "|") to do that.
#    Save the created tibble into `securities_sectored`.
#    Then, select the rows that actually meet the requirements above by deleting the
#    rows that are in `securities_sectored`.
#    You can use `anti_join()` on `securities` and `securities_sectored`.
#    Before `anti_join`, remember to convert `GICS Sector` of `securities`
#    to factor to be consistent.
#    Save the result tibble into `securities_selected`.
...
securities<-securities%>%
  mutate(`GICS Sector`=factor(`GICS Sector`,unique(`GICS Sector`)))
securities_sectored<-securities%>%
  mutate(`GICS Sector truncated`=fct_infreq(`GICS Sector`))%>%
  mutate(`GICS Sector truncated`=fct_lump(`GICS Sector truncated`,n=6))%>%
  filter(`GICS Sector truncated` == "Other")%>%
  filter(!(str_detect(`GICS Sub Industry`,"REITs")|`GICS Sub Industry`=="Gold"))
...
fund_time<-fund%>%
  ...
  mutate(`Period Ending Year`=str_replace_all(`Period Ending`,"/","-"))%>%
  mutate(`Period Ending Year`=mdy(`Period Ending Year`,tz="America/New_York"))%>%
  mutate(`Period Ending Year`=year(`Period Ending Year`))%>%
  drop_na()%>%
...
#    # … with 1,289 more rows, and 1 more variable: `Gross Margin` <dbl>
## Do not modify this line!
...
library(ggplot2)
gross_margin<-ggplot(data=securities_fund,mapping=aes(x=`Gross Margin`),fill=`Period Ending Year`)+
...
#    # A tibble: 324,240 x 9
#    `Ticker symbol`  close   high date                 year Security
#    <chr>            <dbl>  <dbl> <dttm>              <dbl> <chr>
#    1 AAL               5.12   5.22 2012-01-03 00:00:00  2012 America…
#    2 AAP              69.1   71.6  2012-01-03 00:00:00  2012 Advance…
#    3 ABT              56.7   56.8  2012-01-03 00:00:00  2012 Abbott …
#    4 ADS             103.   105.   2012-01-03 00:00:00  2012 Allianc…
#    5 AEE              32.8   33.7  2012-01-03 00:00:00  2012 Ameren …
#    6 AEP              40.8   42.0  2012-01-03 00:00:00  2012 America…
#    7 AKAM             32.9   33.2  2012-01-03 00:00:00  2012 Akamai …
#    8 ALB              52.7   54.2  2012-01-03 00:00:00  2012 Albemar…
#    9 ALK              73.9   76.7  2012-01-03 00:00:00  2012 Alaska …
#    10 AME              42.2   43.4  2012-01-03 00:00:00  2012 AMETEK …
#    # … with 324,230 more rows, and 3 more variables: `GICS
...
full_stock<-raw_time%>%
  ...
  filter(year==`Period Ending Year`)
full_stock
...
filtered_company<-full_stock%>%
  filter(Security %in% c("Aetna Inc", "Amazon.com Inc", "Facebook", "Whole Foods Market","FedEx Corporation", "Boeing Company", "The Walt Disney Company"))
...
trend<-ggplot(data=filtered_company,aes(x=date,y=close))+
...
return_stock<-full_stock%>%
  ...
  filter(date==status)%>%
  summarize(Return=diff(price)/price[1])%>%
...
summary_stock<-return_stock%>%
  ...
  summarize(mean_return=mean(as.numeric(Return)),
            q1=quantile(as.numeric(Return),probs=0.25),
            q2=quantile(as.numeric(Return),probs=0.75))%>%
